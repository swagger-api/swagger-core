...
class PetResourceTest extends FlatSpec with Matchers {
  it should "find a Post operation with single object" in {
    val swagger = new Reader(new Swagger()).read(classOf[PetResource])
    val petPath = swagger.getPaths().get("/pet")
...
@RunWith(classOf[JUnitRunner])
class PetResourceTest2 extends FlatSpec with Matchers {
