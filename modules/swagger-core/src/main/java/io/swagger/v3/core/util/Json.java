...
public class Json {
    ...
    private static final MapperHolder HOLDER = new MapperHolder(ObjectMapperFactory::createJson);

    private static final Logger LOGGER = LoggerFactory.getLogger(Json.class);

    /**
     * Returns the shared OpenAPI 3.0 JSON mapper. It is built lazily from {@link ObjectMapperFactory} and rebuilt when
     ...
     * the result. This is the Jackson 3 replacement for mutating {@code Json.mapper()} directly, e.g.
     * {@code Json.configure(b -> b.addModule(new KotlinModule.Builder().build()))}.
