...
class ResourceListingIT extends FlatSpec with ShouldMatchers {
  it should "read a resource listing" in {
    ...
    assert((doc.apis.map(api => api.path).toSet & Set("/api-docs/pet", "/api-docs/owner")).size == 2)
  }
  ...
  ignore should "read the pet api description" in {
    ...
    println(json)
    val listing = JsonSerializer.asApiListing(json)
    assert(listing.apis.size === 3)
    assert((listing.apis.map(api => api.path).toSet &
      Set("/pet/{petId}",
        "/pet/{petId}/owner")).size == 2)
  }
